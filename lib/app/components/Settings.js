...
class Settings extends TransitionComponent {
  constructor(id, api, events) {
    ...
    this.sweepstakes = new SweepstakesSelection('sweepstakes', api, events);
    this.registrations = new Registrations('registrations', api, events);
    ...
    events.on('roboto:authorized', user => {
      ...
      events.on('roboto:form-profile-submitted', data => {
        ...
        for (let field in data) {
          ...
          } else if (field === 'gender' || field === 'cable_provider') {
            address.attributes.push({
              name: field,
              value: data[field]
            });
        ...
        .then(resp => {
          Materialize.toast('<i class="material-icons">thumb_up</i>&nbsp;&nbsp; Successfully submitted!', 3000);
